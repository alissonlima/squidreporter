...
class Relatorio1_Model_FiltroData
{

	public function Build($date_begin, $date_end)
	{

               $sql = "SELECT ";
              #$sql.= "log.username  AS name, ";
               $sql.= "CONCAT('<a href=\"/Relatorio1/Index/log-user/username/',log.username,'/usergroup/',grp.name,'/begin/{$date_begin}/end/{$date_end}\">',log.username,'</a>') AS name, ";
...
#              $sql.= "SUM(DATE_FORMAT(log.date_time,'%d/%m/%Y')) AS 'tempo gasto' ";
	       ...
	       $grid = new Application_Model_Grid();
              # $buffer = "<h3>Relatorio1 de {$date_month} - {$date_year}</h3>\n";
        ...
        {      
               ...
               $sql = "SELECT ";
               $sql.= "CONCAT('<a href=\"/Relatorio1/Index/log-detailed/detailed/',domain_of_url(`request_url`),'/username/{$username}/date_begin/{$date_begin}/date_end/{$date_end}\">detailed</a>') AS detailed, ";
